...
@property (readwrite, nonatomic) DITMessagingPeer *peer;
...
//    self.client = [[DITClient alloc] initWithHostname:<#(NSString *)#> port:<#(unsigned short)#>
    }
...
- (void)netServiceDidResolveAddress:(NSNetService *)sender
    ...
    self.client = [[DITClient alloc] initWithHostname:sender.hostName port:sender.port];
    [self.client connect:^(NSError *error) {

        NSLog(@"CONNECTED");

        NSDictionary *theMessageHandlers = @{
            @"gyro_update": ^(DITMessagingPeer *inPeer, DITMessage *inMessage, NSError **outError) {
//                NSLog(@"%@ %@", inPeer, inMessage);
        ...
        self.peer = [[DITMessagingPeer alloc] initWithSocket:self.client.socket messageHandlers:theMessageHandlers];
        }];
    }
