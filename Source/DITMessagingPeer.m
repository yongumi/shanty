...
@implementation DITMessagingPeer
...
//            NSLog(@"READ");
        ...
        [self addCommand:@"HELLO" handler:^(DITMessagingPeer *inPeer, DITMessage *inMessage, NSError **outError) {
            ...
            DITMessage *theResponse = [[DITMessage alloc] initWithControlData:theControlData metadata:NULL data:NULL];
            [strong_self sendMessage:theResponse replyBlock:NULL];

            NSLog(@"%@", inMessage);
            return(YES);
            }];
        [self addCommand:@"HELLO_RESPONSE" handler:^(DITMessagingPeer *inPeer, DITMessage *inMessage, NSError **outError) {
            NSLog(@"%@", inMessage);
            return(YES);
            }];
...
- (void)sendMessage:(DITMessage *)inMessage replyBlock:(MessageHandlerBlock)inBlock
    ...
    DITMessage *theMessage = [[DITMessage alloc] initWithControlData:theControlData metadata:inMessage.metadata data:inMessage.data];
...
- (void)read
    {
    DITDataScanner *theDataScanner = [[DITDataScanner alloc] initWithData:self.data];
...
- (BOOL)_handleMessage:(DITMessage *)inMessage error:(NSError *__autoreleasing *)outError
    {
